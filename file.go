...
func getPbGoFileByProtoDir(protoDir string) ([]string, error) {
	res := make([]string, 0)
	...
	walkDir = func(dirPath string) error {
		...
		for _, file := range files {
			if file.IsDir() {
				if err = walkDir(dirPath + "/" + file.Name()); err != nil {
					return err
				}
			} else {
				if strings.HasSuffix(file.Name(), "pb.go") {
					res = append(res, dirPath+"/"+file.Name())
				}
			}
		}
		return nil
	}
	if err := walkDir(protoDir); err != nil {
		return nil, err
	}
	return res, nil
}